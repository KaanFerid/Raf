...
class BookRow(Adw.ActionRow):
    def __init__(self, book, is_installed=False, main_window=None):
        ...
        self.set_subtitle(
            f"{self.book['publisher']} • {tr('ui.type_label', type=file_type_str)}"
        )
    ...
    def update_status(
        self, is_installed, downloading=False, percent=0, speed_str="", is_offline=False
    ):
        self.is_installed = is_installed
    ...
    def retranslate_ui(self, is_offline=False):
        self.set_subtitle(
            f"{self.book['publisher']} • {tr('ui.type_label', type=self.type_str)}"
        )
        self.update_status(
            self.is_installed,
            self.downloading,
            self.progress_bar.get_fraction() * 100,
            self.speed_label.get_text(),
