...
def show_message(parent, title, text, type="info"):
    dialog = Adw.MessageDialog(transient_for=parent, heading=title, body=text)
...
class DesktopEditorDialog(Adw.Window):
    ...
    def load_desktop_file(self):
        ...
        path_to_load = (
            self.local_path if os.path.exists(self.local_path) else self.global_path
        )
    ...
    def save_desktop_file(self, btn):
        ...
        if not new_name:
            show_message(
                self,
                tr("ui.error"),
                tr("ui.name_empty_error", default="Name cannot be empty."),
                type="error",
            )
            return
        ...
        try:
            ...
            show_message(
                self,
                tr("ui.error"),
                tr("ui.desktop_editor_save_failed", error=str(e)),
                type="error",
            )
