...
def get_disk_info():
    ...
    try:
        ...
        return tr(
            "ui.system_free_space", free=f"{free_gb:.1f}", total=f"{total_gb:.1f}"
        )
    except Exception:
...
class PreferencesWindow(Adw.PreferencesWindow):
    ...
    def init_ui(self):
        ...
        theme_model = Gtk.StringList.new(
            [tr("ui.system_theme"), tr("ui.light_theme"), tr("ui.dark_theme")]
        )
        ...
        if mode == "system":
            ...
            self.theme_combo.set_selected(1)
        elif mode == "dark":
            self.theme_combo.set_selected(2)
        ...
        update_model = Gtk.StringList.new(
            [
                tr("ui.update_policy_manual"),
                tr("ui.update_policy_check"),
                tr("ui.update_policy_auto"),
            ]
        )
        ...
        if policy == "off":
            self.update_combo.set_selected(0)
        elif policy == "check":
            self.update_combo.set_selected(1)
        elif policy == "auto":
            self.update_combo.set_selected(2)
        ...
        if os.path.exists(cache_dir):
            try:
                ...
                show_message(
                    self,
                    tr("ui.error"),
                    tr("ui.cache_clear_error", error=str(e)),
                    type="error",
                )
